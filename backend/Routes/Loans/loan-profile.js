...
router.get('/transactions_loan', authenticateUser, async (req, res) => {
  try {
    ...
    const updatedTransactions = transactions.map(loanDoc => {
      const loan = loanDoc.toObject();
      if (loan.status === 'active' && loan.loanDetails) {
        ...
        loan.loanDetails.totalAmount = remainingPrincipal + totalInterest;
        loan.loanDetails.paidInterestTotal = paidInterestTotal;
      }
      return loan;
    });
...
// Add this new endpoint for Top-Down from Interest
router.put('/top-down-interest/:customerID', async (req, res) => {
  const { customerID } = req.params;
  ...
  try {
    const loan = await Loan.findOne({ customerID: customerID });
    if (!loan) {
    ...
    const { topUpInterest, topdownInterest, totalInterest } = calculateTopUpInterest(
      loan.loanDetails.amount, 
      loan.loanDetails.interestRate, 
      loan.loanDetails.startDate, 
      topUpHistory, 
    ...
    // Check if there's enough interest to deduct
    if (currentTotalInterest <= 0) {
      return res.status(400).json({ 
        error: 'No interest available to deduct. Current interest is zero.',
        currentInterest: currentTotalInterest 
      });
    }

    // Check if payment amount exceeds available interest
    ...
    // Add to interest payment history (new array)
    if (!loan.loanDetails.interestPaymentHistory) {
      loan.loanDetails.interestPaymentHistory = [];
    }

    loan.loanDetails.interestPaymentHistory.push({
...
// calculateTopUpInterest was here - now in utils/interestCalculator.js

// ============================================
// UPDATE the interest update endpoint
// ============================================
...
schedule.scheduleJob('0 0 * * *', async () => {
  ...
  try {
    ...
    console.log('Interest updated successfully for all active loans.');
  } catch (err) {
...
// Simple Top-Up Endpoint (Directly adds to principal)
router.put('/top-up/:customerID', async (req, res) => {
  ...
  try {
    ...
    // Update the loan amount (add to base principal for this specific endpoint)
