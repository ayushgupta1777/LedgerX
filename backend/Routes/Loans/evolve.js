...
router.put('/top-down/:customerID', async (req, res) => {
  ...
  try {
    ...
    // Calculate remaining principal dynamically to avoid stale DB values
    const dynamicTopUpTotal = loan.loanDetails.topUpHistory?.reduce((sum, td) => sum + (parseFloat(td.amount) || 0), 0) || 0;
    const totalPrincipal = loan.loanDetails.amount + dynamicTopUpTotal;
    const totalRepaid = loan.loanDetails.topDownHistory?.reduce((sum, td) => sum + (parseFloat(td.amount) || 0), 0) || 0;
    const remainingPrincipal = totalPrincipal - totalRepaid;

    // Check if there's enough principal to deduct
    if (remainingPrincipal <= 0) {
      return res.status(400).json({ 
        error: 'No principal remaining to deduct.',
    ...
    if (amount > remainingPrincipal) {
      return res.status(400).json({ 
        error: `Payment amount (₹${amount}) exceeds remaining principal (₹${remainingPrincipal.toFixed(2)})`,
        remainingPrincipal: remainingPrincipal 
      });
    }

    // Add to principal payment history
    if (!loan.loanDetails.topDownHistory) {
      loan.loanDetails.topDownHistory = [];
    }

    loan.loanDetails.topDownHistory.push({
    ...
    res.json({ 
      ...
      paidAmount: amount,
      remainingPrincipal: remainingPrincipal - amount,
      loan 
    });

  } catch (err) {
    console.error('Error processing principal payment:', err);
    res.status(500).json({ error: 'Failed to process principal payment.' });
  }
});
